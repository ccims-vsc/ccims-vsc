...
export interface QuickPickParameters<T extends QuickPickItem> extends InputParameters {
	items: T[];
	onDidChangeValue?: (e: string, input: QuickPick<T>) => any;
	activeItem?: T;
	placeholder: string;
}

export interface InputBoxParameters extends InputParameters {
	value: string;
	prompt?: string;
	password?: boolean;
	...
	async showQuickPick<T extends QuickPickItem, P extends QuickPickParameters<T>>({ title, step, totalSteps, items, activeItem, placeholder, buttons, shouldResume, onDidChangeValue }: P) {
		...
		try {
			return await new Promise<T | (P extends { buttons: (infer I)[] } ? I : never)>((resolve, reject) => {
				...
				if (onDidChangeValue != undefined) {
					disposables.push(
						input.onDidChangeValue(text => onDidChangeValue(text, input))
					);
				}
				if (this.current) {
					this.current.dispose();
